<script lang="ts">
  ...
  let recentItems = $derived(
    recentIds.map(id => $activeCatalog.find(f => f.id === id)).filter(Boolean) as FurnitureDef[]
  );
  ...
  let favoriteItems = $derived(
    favoriteIds.map(id => $activeCatalog.find(f => f.id === id)).filter(Boolean) as FurnitureDef[]
  );
      ...
      <div class="space-y-2">
        {#if $activeCatalog.length === 0}
          <div class="flex flex-col items-center justify-center py-12 px-4 text-center space-y-3">
            <div class="w-10 h-10 border-2 border-blue-100 border-t-blue-500 rounded-full animate-spin"></div>
            <div>
              <p class="text-sm font-medium text-gray-700">Loading catalog...</p>
              <p class="text-[10px] text-gray-400 mt-1">Objects are being synchronized from the host application.</p>
            </div>
          </div>
        {:else}
          ...
          <div class="flex flex-wrap gap-1 max-h-24 overflow-y-auto">
            ...
            {#each $activeFurnitureCategories as cat}
              <button
              ...
              <div class="col-span-2 py-8 text-center">
                ...
                {#if search || selectedCategory !== 'All'}
                  <button 
                    ...
                    onclick={() => { search = ''; selectedCategory = 'All'; }}
