...
export const isReadOnly = writable<boolean>(false);
...
/** Reactive store exposing undo history */
...
export function addStair(position: Point): string {
  ...
  mutate((f) => {
    ...
    f.stairs.push({ id, position, rotation: 0, width: 100, depth: 300, height: 260, riserCount: 14, direction: 'up', stairType: 'straight' });
  }, 'Added stair');
  return id;
}
